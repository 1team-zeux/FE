...
import {
  useSliMetricsStream, useSystemMetricsStream, useIncidentQuery,
...
// ── Queries ────────────────────────────────────────────────────────
const { data: detail, error: streamError } = useSliMetricsStream(svcId, tenantId, isLive);
// 데이터 있으면 재연결 중이어도 차트 유지 — error는 초기 로드 실패에만 표시
const isLoading = computed(() => !detail.value && !streamError.value);
const isError   = computed(() => !detail.value && !!streamError.value && streamError.value !== '연결 중...');

const { data: systemMetrics } = useSystemMetricsStream(svcId, tenantId, isLive);
const { data: incident, isLoading: isLoadingIncident } = useIncidentQuery(svcId, tenantId || undefined);
...
const handleTrace = () => { activeTab.value = 'trace'; };
  ...
  <div class="py-8 px-8">
    ...
    <template v-else-if="detail">
      ...
      <div class="mb-5 flex items-center justify-between">
        <div>
          <div class="text-sm font-bold text-brand uppercase tracking-widest mb-1">Level 3 · 서비스 상세</div>
        ...
        <div class="flex items-center gap-2">
          <button
            class="flex items-center gap-2 px-4 py-2 rounded-md text-sm font-bold border transition-colors"
            :class="isLive ? 'bg-brand text-white border-brand' : 'bg-white text-gray-500 border-border hover:bg-gray-50'"
            @click="isLive = !isLive"
          >
            <span class="w-2 h-2 rounded-full transition-colors"
              :class="streamError === '연결 중...' ? 'bg-yellow-300 animate-pulse' : isLive ? 'bg-white animate-pulse' : 'bg-gray-400'" />
            {{ streamError === '연결 중...' ? '재연결 중...' : isLive ? 'LIVE' : 'Stop' }}
          </button>
      ...
      <div v-if="activeTab === 'metrics'">
        ...
        <div class="mb-10 border-l-4 border-brand pl-5">
          ...
          <div class="flex items-center gap-3 mb-3 -ml-5 pl-4 py-2.5 bg-brand/5 rounded-r-lg border-b border-brand/10">
            <span class="text-sm font-bold text-brand uppercase tracking-widest">Application Layer</span>
            <span class="text-sm text-gray-400">SLI · Error Budget · 이상 알람</span>
          </div>

          <!-- SLI Chip Filter (dedicated row) -->
          <div class="flex flex-wrap gap-2 mb-4">
            <span class="text-sm font-bold text-gray-400 self-center mr-1">지표 필터</span>
            <button
              v-for="sli in detail.slis" :key="sli.id"
              class="px-3 py-1 text-sm font-bold rounded-full border transition-all"
          ...
          <div class="grid grid-cols-1 xl:grid-cols-[1fr_340px] gap-6 items-start">
            ...
            <div class="bg-bg-card border border-border rounded-lg overflow-hidden shadow-sm">
              <div class="bg-gray-50 border-b border-border p-3 flex justify-between items-center text-sm">
              ...
              <div v-for="sli in filteredSlis" :key="sli.id" class="border-b border-border last:border-b-0">
                <div class="p-3 flex justify-between items-center">
                  <div class="flex items-center gap-2">
                    ...
                    <span class="px-1.5 py-0.5 bg-gray-100 text-sm font-bold rounded text-gray-500">→ {{ sli.sla }} SLA</span>
                  </div>
                  <div class="font-mono text-sm font-bold">{{ sli.series.at(-1) }}{{ sli.unit }}<span class="text-sm text-gray-400 font-normal ml-1">Current</span></div>
                </div>
                <LineChart
              ...
              <div class="bg-gray-50/30">
                <div class="p-3 flex justify-between items-center">
                  ...
                  <div class="font-mono text-sm font-bold text-status-critical">{{ detail.budget.series.at(-1) }}%<span class="text-sm text-gray-400 font-normal ml-1">Remaining</span></div>
                </div>
              ...
              <div class="flex justify-between px-3 py-2 text-sm font-mono text-gray-400 border-t border-border">
            ...
            <div class="sticky top-20 space-y-3">
              <div class="text-sm font-bold text-gray-400 uppercase tracking-widest mb-1">이상 이벤트</div>
              <div
              ...
              >
                <div class="flex items-center gap-2 px-3 py-2.5">
                  ...
                  <span class="font-mono text-sm font-bold text-gray-500 shrink-0">{{ a.t }}</span>
                  <span class="text-sm font-bold flex-1 truncate" :class="a.sev === 'critical' ? 'text-status-critical' : 'text-status-warning'">{{ a.title }}</span>
                  <span class="text-sm font-bold px-1.5 py-0.5 rounded shrink-0" :class="a.sev === 'critical' ? 'bg-status-critical/10 text-status-critical' : 'bg-status-warning/10 text-status-warning'">
                ...
                <div v-if="activeAlarm?.id === a.id" class="px-3 pb-3 border-t border-border/50 pt-2.5">
                  ...
                  <div class="flex gap-2">
                    <button class="flex-1 py-1.5 bg-brand text-white text-sm font-bold rounded-md hover:brightness-110 transition-all" @click.stop="handleTrace">Trace →</button>
                    <button class="px-3 py-1.5 border border-brand text-brand text-sm font-bold rounded-md hover:bg-brand/5 transition-all" @click.stop="activeTab = 'rca'">RCA →</button>
                  </div>
                </div>
              </div>
            </div>
        ...
        <div v-if="systemMetrics" class="border-l-4 border-amber-400 pl-5">
          <div class="flex items-center gap-3 mb-4 -ml-5 pl-4 py-2.5 bg-amber-50/50 rounded-r-lg border-b border-amber-200/50">
            <span class="text-sm font-bold text-amber-600 uppercase tracking-widest">Host Layer</span>
            <span class="text-sm text-gray-400">CPU · Memory · Disk · Network</span>
          </div>
      ...
      <div v-else-if="activeTab === 'trace'">
        <div class="mb-4 px-4 py-2.5 bg-gray-50 border border-border rounded-lg text-sm text-gray-500">
      ...
      <div v-else-if="activeTab === 'rca'">
        ...
        <div class="flex flex-wrap gap-2 mb-6">
          <button
            v-for="a in detail.alarms" :key="a.id"
            class="flex items-center gap-2 px-4 py-2.5 rounded-lg border text-sm font-bold transition-all"
            :class="{
          ...
          >
            ...
            <span class="text-sm px-1.5 py-0.5 rounded border font-bold" :class="a.sev === 'critical' ? 'bg-status-critical/10 border-status-critical/30 text-status-critical' : 'bg-status-warning/10 border-status-warning/30 text-status-warning'">
              {{ a.sev.toUpperCase() }}
            </span>
          </button>
        </div>
        <div v-if="isLoadingIncident" class="p-8 text-center text-gray-400">RCA 데이터 로드 중...</div>
        <RcaPanel v-else-if="incident" :incident="incident" />
      </div>
