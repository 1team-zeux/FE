...
<template>
  <div class="card relative bg-bg-card border border-border rounded-lg p-4 transition-all hover:border-brand hover:-translate-y-1 cursor-pointer overflow-hidden shadow-sm" @click="$emit('click')">
    ...
    <div class="flex justify-between items-start mb-4">
      <div>
        <div class="text-[10px] text-gray-400 font-bold uppercase tracking-wider">{{ bu.customer }}</div>
        <div class="text-base font-bold text-text-primary mt-1">{{ bu.bu }}</div>
        <div class="text-[10px] text-gray-400 mt-0.5">{{ bu.platform }}</div>
      </div>
    ...
    <div class="flex gap-2 mb-4">
      <span class="px-2 py-0.5 bg-gray-100 text-[10px] font-bold rounded text-gray-500">Services {{ bu.serviceCount }}</span>
      <span v-if="bu.riskCount > 0" class="px-2 py-0.5 bg-status-critical/10 text-[10px] font-bold rounded text-status-critical border border-status-critical/20">Risks {{ bu.riskCount }}</span>
      <span v-else class="px-2 py-0.5 bg-status-ok/10 text-[10px] font-bold rounded text-status-ok border border-status-ok/20">Healthy</span>
    </div>
    <div class="mb-4">
    ...
    <div class="pt-3 border-t border-border flex items-start gap-2">
      <div class="w-1.5 h-1.5 rounded-full mt-1.5" :class="{ 'bg-status-critical': bu.status === 'critical', 'bg-status-warning': bu.status === 'warning', 'bg-status-ok': bu.status === 'healthy' }" />
      <div class="text-[11px] text-gray-500 leading-tight">{{ bu.note }}</div>
    </div>
    <div v-if="bu.drillable" class="mt-3 text-[11px] text-brand font-bold flex items-center gap-1">Start Drill-down →</div>
  </div>
</template>
