  ...
  <div class="card relative bg-bg-card border border-border rounded-lg p-4 transition-all hover:border-brand hover:-translate-y-1 cursor-pointer overflow-hidden shadow-sm" @click="$emit('click')">
    ...
    <div class="flex justify-between items-start mb-3">
      <div class="flex-1 min-w-0">
        <div class="text-sm text-gray-400 font-bold uppercase tracking-wider truncate">{{ bu.customer }}</div>
        <div class="text-lg font-bold text-text-primary mt-0.5 truncate">{{ bu.bu }}</div>
        <div class="text-sm text-gray-400 mt-0.5 truncate">{{ bu.platform }}</div>
      </div>
      <div class="flex flex-col items-end gap-1 ml-2 shrink-0">
        ...
        <span class="px-1.5 py-0.5 bg-gray-100 text-xs font-bold rounded text-gray-500">{{ bu.tier }}</span>
      </div>
    </div>
    ...
    <div class="mb-3">
      <div class="flex justify-between items-center mb-1">
        <span class="text-sm font-bold text-gray-400 uppercase tracking-wider">Risk Score</span>
    ...
    <!-- Error Budget -->
    <div class="mb-3">
      <div class="flex justify-between text-sm text-gray-400 mb-1"><span>Error Budget</span><span class="font-bold">{{ bu.lowestBudget }}% left</span></div>
      <div class="h-1.5 bg-gray-100 rounded-full overflow-hidden">
    ...
    <div class="flex gap-2 mb-3">
      <span class="px-2 py-0.5 bg-gray-100 text-sm font-bold rounded text-gray-500">서비스 {{ bu.serviceCount }}개</span>
      <span v-if="bu.activeEvents > 0" class="px-2 py-0.5 bg-status-critical/10 text-sm font-bold rounded text-status-critical border border-status-critical/20">이벤트 {{ bu.activeEvents }}건</span>
      <span v-else class="px-2 py-0.5 bg-status-ok/10 text-sm font-bold rounded text-status-ok border border-status-ok/20">이벤트 없음</span>
    </div>
    ...
    <div class="pt-3 border-t border-border flex items-start gap-2">
      ...
      <div class="text-sm text-gray-500 leading-tight">{{ bu.note }}</div>
    </div>
    <div v-if="bu.drillable" class="mt-3 text-sm text-brand font-bold flex items-center gap-1">Drill-down →</div>
  </div>
</template>
